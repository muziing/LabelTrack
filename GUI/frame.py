...
from demo.bytetrack import Predictor, frames_track
    ...
    def update_status(self, message):
        ...
        if re.search(r".*(0).+/.*", message):
            self.frameLabel.setPixmap(
                QtGui.QPixmap.fromImage(img_cv_to_qt(self.imgFrames[0]))
            )
...
class trackWorker(QThread):
    ...
    def track_frame(self):
        ...
        self.imgFrames = frames_track(
            exp, predictor, self.imgFrames, cfg, self.sinOut, self.frame
        )
